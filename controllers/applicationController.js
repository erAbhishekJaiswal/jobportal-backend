...
exports.getApplicationsByUser = async (req, res) => {
  try {
    ...
    .select('-coverLetter -portfolio -linkedin -salaryExpectation -noticePeriod -source -__v -resumeUrl -publicId') // Exclude sensitive fields
    .populate({path: 'job', salect: 'title company location description', populate:{
      path:'company',
      model:'Company',
      select:'name'
    }})
...
exports.getDetailsOfApplication = async (req, res) => {
  try {
    const { id } = req.params;
    const application = await Application.findById(id).populate('job', 'title company location description');
    if (!application) {
