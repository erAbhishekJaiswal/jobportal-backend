...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        const token = signAccessToken({ id: user._id, role: user.role , email: user.email});
        ...
        res.status(200).json({ token, role: user.role,email: user.email, message: "Login successful" });
    } catch (error) {
        console.error(error);
